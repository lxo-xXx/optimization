#!/usr/bin/env python3
"""
Heat Recovery Process Optimization - Fixed Python Implementation
...
class ORCOptimizer:
    ...
    def simple_optimization(self, fluid: WorkingFluid, config: str = 'A') -> Dict:
        ...
        # More reasonable temperature range - pinch point constraint was too restrictive
        T_evap_max = min(420, self.T_hw_in - 20)  # Allow reasonable approach to hot water
        T_evap_range = [i for i in range(int(self.T_cond + 10), int(T_evap_max), 5)]
        m_wf_range = [0.5 + i*0.2 for i in range(100)]  # 0.5 to 20.5 kg/s
        ...
        for T_evap in T_evap_range:
            for m_wf in m_wf_range:
                ...
                # Power calculations (simplified but more realistic)
                W_turb = m_wf * fluid.cp_avg * (T_evap - self.T_cond) * self.eta_turb
                W_pump = m_wf * 2.0 / self.eta_pump  # More realistic pump work
                
                if config == 'B':
                    # Configuration B with recuperator
                    Q_recup = min(W_turb * 0.2, m_wf * fluid.cp_avg * 20)  # Limited recuperation
                    efficiency_improvement = 1.12  # 12% improvement from recuperator
                ...
                # Heat balance constraint - must match available heat
                heat_error = abs(Q_evap_required - self.Q_available) / self.Q_available
                if heat_error > 0.05:  # Allow 5% error
                    continue
        ...
        for fluid_name, fluid in self.fluids.items():
            ...
            if result['success']:
                ...
                if config == 'B':
                    ...
                    print(f"{fluid_name:>8}: {power:7.1f} kW, η={efficiency:5.2f}%, T={temp:5.1f}°C, ṁ={mass_flow:4.1f} kg/s, Recup={recup:5.1f} kW")
                else:
                    print(f"{fluid_name:>8}: {power:7.1f} kW, η={efficiency:5.2f}%, T={temp:5.1f}°C, ṁ={mass_flow:4.1f} kg/s")
    ...
    def generate_report(self, config_a_results: Dict, config_b_results: Dict):
        ...
        with open('optimization_report.txt', 'w') as f:
            ...
            if best_a_fluid:
                ...
                f.write(f"- Evaporation temperature: {config_a_results[best_a_fluid]['T_evap']-273.15:.1f}°C\n")
                f.write(f"- Working fluid mass flow: {config_a_results[best_a_fluid]['m_wf']:.2f} kg/s\n\n")
            else:
            ...
            if best_b_fluid:
                ...
                f.write(f"- Evaporation temperature: {config_b_results[best_b_fluid]['T_evap']-273.15:.1f}°C\n")
                f.write(f"- Working fluid mass flow: {config_b_results[best_b_fluid]['m_wf']:.2f} kg/s\n\n")
            else:
            ...
            if best_a_fluid and best_b_fluid:
                ...
                f.write("Recommendation: Configuration B is the only feasible option.\n")
            elif best_a_fluid:
                f.write("Recommendation: Configuration A is the only feasible option.\n")
            else:
                f.write("No feasible solutions found. Review problem constraints.\n")
...
def main():
    ...
    if best_a_fluid and best_b_fluid:
        ...
        print("No feasible solutions found - check problem constraints")
